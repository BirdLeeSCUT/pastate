...
- Module3 是具有一些信息需要在其他模块同步显示，如聊天列表要告诉导航栏目前多少条未读信息，其 store3 还被 view2 依赖；
- Module4 是系统的公用模块，如登录模块，其状态要被其他模块获知。

模块之间的互相依赖分为两种：
1. state 依赖：需要使用到其他模块的 state, 如根据用是否已登录实现不同的视图渲染或actions处理
2. actions 依赖：需要触发其他应用的 actions, 如触发登录模态窗或通知模态窗


因此，在多模块应用中，我们不再简单地使用 makeOnlyContainer 把 Component 和 store (state) 一对一绑定，而是先把各个模块 store 连接一棵 store 树, 如下：  
...
然后指定 Component 引用这颗 store 树的**哪些**（一个或多个）节点的 state， 生成 Container：  
...
首先，在 `src/index.js` 中创建一个 store 树, 我们使用普通的对象模式来描述这颗 store 树：
...
Pastate 的 storeTree 支持多层管理模式，你可以使用下面这个简单的格式创建多层嵌套的 storeTree：
```javascript
...
const storeTree = {
    student: StudentPanel.store,
...
...
```  
在容器定义中，你可以简单的 '.'点格式来指明对这些多层嵌套内的 store 的引用：
```javascript
...
## 多模块组合
在实际应用中，我们一般会使用一个具有 “导航属性” 的容器作为**根容器**来控制应用多个模块的显示，在我们班级信息管理系统中就是导航栏模块 Navigator ，我们来看看他如何来控制其他模块的显示：
...
...
```
在 `Navigator.view.jsx` 中，我们引入另外两个模块的视图容器，并根据 state.selected 的值进行渲染：
...
假设没有 storeTree, 我们一般会想到 “冗余数据” 方法来实现这个功能：在 Navigator 的 state 中定义两个值来分别表示学生数和课程数，并定义对于的修改方法给其他模块来修改这两个数量，大致如下：
```javascript
const initState = {
    /** @type {'student' | 'class'} */
    selected: 'student',
...
// NOTE: 以上为不推荐的实现方式
```
如果你不是使用 pastate 或 redux 等独立于 react component 的 state 管理管理框架，在多个组件互相配合联动时或许不得不使用这种模式。但**这种模式具有较大地缺陷**：当应用日益复杂，你会发现当你做一个简单的操作后，需要去更新很多相关的冗余数据，而且，你很容易忘了去完备地更新冗余数据或重复更新冗余数据。这样一来，应用变得难以开发、修改和维护。

因此，我们需要使用 “唯一数据源 ([SSOT](https://en.wikipedia.org/wiki/Single_source_of_truth), Single source of truth)” 模式来实现多组件的协作，pastate 的 storeTree 正是唯一数据源模式的一种实现方式，下面，我们来看看如何实现：  

基本原理：学生和课程的数量信息在学生和课程板块已经包含，如果有哪个地方需要用到这些数量信息，则在那个地方通过 “唯一源数据” 来 “计算” 出来。
...
```
makeContainer 的第二个参数其实是一个 props mapper, 你可以通过一个 map 函数来定义把 store 树的那些 state 节点映射到 Navigator 组件的 props 上。我们之前使用的 `makeContainer(Navigator, 'nav')` 是一种简写，等价于下面的形式：
```javascipt
makeContainer(Navigator, state => ({
    state: state.nav
}))
```
我们在 render 函数中这样简单地获取 props 中映射入的值：
...
```
这种模式直接从唯一数据源获取 / 计算出与其他模块的 state 相关的数据，使我们避免了容易出问题的“冗余数据” 的更新操作，我们一般把这种从唯一数据源获取 / 组合 / 计算出与其他模块数据称为衍生数据。  

pastate 容器包含了按需渲染逻辑，当且仅当任何一个以上的 props "根属性" 节点的值改变时，pastate 会控制组件视图进行重新渲染。

## 缓存 storeTree 的衍生数据
如果你使用下面这种模式映射 Navigator 组件的 props, 会导致每次学生和课程的元素内部发生修改但学生和课程数量都没有改变时也触发 Navigator 组件的渲染动作：
```javascipt
...
```
这种情况下，每次在计算生成 count 节点时，都会动态计算出一个匿名的 `{student:..., class:...}` 对象，假设 count 节点的计算逻辑比较消耗资源，或者 Navigator 的渲染逻辑比较消耗资源的情况下，这会使应用的性能变差。我们实现一种 storeTree 的衍生数据的缓存逻辑，实现**可记忆**的衍生数据。Pastate 提供 `makeCacheable` 函数实现这个功能：
```javascript
import { makeContainer, makeCacheable } from 'pastate';

// 这是一个具有记忆功能的生成 {student:..., class:...} 对象的函数 / 选择器
...
```
那么当学生模块的 state.selected 的值发生改变时（在实际使用过程中，这种值确实会频繁发生改变），都会重新重新触发发数组 map 函数，这是一个性能隐患。我们引入 makeCacheable 可以避免这个问题：
```javascript
...
Pastate 的 makeCacheable 函数的功能与 [reselect](https://github.com/reactjs/reselect) 库的基础功能类似，用起来比较简单。如果 makeCacheable 不能你的需求，可以看看 reselect 库。
...
当应用**非常简单**时, 在操作上可以直接的把公共模块的 store 引入到其他模块的 model 文件中，并简直接使用公共模块的 state：
...
- 你很容易不知道公共 state 在哪里被怎么改变了；
- 当你需要更新公共 state 的定义时，需要同时去更新多个模块的 actions 。

这使得模块之间的耦合性增强，使得应用难以升级维护。特别是当不同模块是由不同开发者开发的时候，管理这个应用简直就是恶梦。

因此，我们需要使用面向对象的思维与实践模式，把模块看成是一个对象，把模块的 state 看成是模块的私有（private）成员， 只有在模块内部才可以访问和修改。如果其他模块的 actions 需要引用或修改本模块的 state,  请把所需的引用或修改逻辑封装在本模块的 actions 或 actions.public 中，供其他模块使用：
...
const actions = {
    handleClick(){ ... },
    openLoginModal(){ ... }, // 封装并命名 state 操作逻辑
    isLogined(){ return state.isLogined } // 封装并命名 state 节点引用逻辑
}
...
这样中 public actions 模式定义了一种 state 外部访问和操作的授权机制；每个 state 的外部访问和操作的具体实现都是在模块自身内部完成的，增加了模块的内聚性；每个state 的外部访问和操作逻辑都有具体的命名，使模块协作逻辑更加明确。 

其实，public actions 是一种 **“面向接口([Interface](https://en.wikipedia.org/wiki/Interface_(computing)))编程”** 或 **“面向协议([Protocol](https://en.wikipedia.org/wiki/Protocol_(object-oriented_programming)))编程”** 的设计模式。外部模块只需把接口当成一种抽象的联系方式，只需要知道接口的功能和接口参数/返回值，不需要知道接口内部的实现逻辑，而且接口的具体实现逻辑是允许改变的。

最后提个醒，不建议使用 public actions 去同步多个模块的冗余数据，你应该尽量避免冗余数据。